#!/usr/bin/env python3
"""
Experiment 9: Proper Goodfire Replication at Paper Scale (Gemma-3-12B-IT)
=========================================================================
Same methodology as Exp 8, but on the paper's actual model: Gemma-3-12B-IT.
Runs on A100 80GB in bf16 (no quantization).

Exp 8 showed: data effect = +0.187, architecture effect = -0.017.
This experiment isolates the MODEL SCALE effect by running identical
methodology on the 12B model.

Paper setup (what we're matching):
  - Model: Gemma-3-12B-IT (48 layers, hidden_dim=3840)
  - Loc probe: L=4 Transformer, E=128, Nh=8
  - Cls probe: E=2048, Nh=16 (paper spec for 3840-dim model)
  - Layers: L20 (localization), L[20,30] (classification)
  - Data: 50 prompts x 2 completions (same as exp8 for fair comparison)

What's different from exp8:
  - Model: Gemma-3-12B-IT (48L, 3840D) vs Gemma-2-2B-it (26L, 2304D)
  - Cls probe: E=2048, Nh=16 (paper spec) vs E=1024, Nh=8 (scaled down)
  - Layers: L20, L[20,30] (paper's actual layers) vs L11, L[11,16]
  - Hardware: A100 80GB vs RTX 4070 Ti 16GB

Usage: python exp9_12b_replication.py
Output: results/exp9_results.md + results/exp9_results.json
"""
...
OUTPUT_JSON = REPO_ROOT / "results" / "exp9_results.json"
OUTPUT_MD = REPO_ROOT / "results" / "exp9_results.md"
CACHE_COMPLETIONS = REPO_ROOT / "results" / "exp9_completions.json"
CACHE_ENTITIES = REPO_ROOT / "results" / "exp9_entities.json"
CACHE_ACTIVATIONS = REPO_ROOT / "results" / "exp9_activations.pkl"

# Paper's actual layer config (48-layer model)
LOC_LAYER = 20
CLS_LAYERS = [20, 30]
HIDDEN_DIM = 3840  # Gemma-3-12B-IT hidden size

# Localization probe (paper B.3.1) — same as exp8
LOC_EMBED = 128
LOC_HEADS = 8
LOC_DEPTH = 4
LOC_WINDOW = 256

# Classification probe (paper B.3.2) — paper's full spec for 3840-dim
CLS_EMBED = 2048   # paper spec (was 1024 on 2B)
CLS_HEADS = 16     # paper spec (was 8 on 2B)
...
GEN_TOP_P = 0.95
GEN_TOP_K = 64
GEN_MAX_TOKENS = 1024  # Longer than exp8 (512) — 12B generates better
...
# ---------------------------------------------------------------------------
# Prompts: same 50 as exp8 for fair comparison
# ---------------------------------------------------------------------------
PROMPTS = [
    # Biography (8)
...
# ---------------------------------------------------------------------------
# Model loading — Gemma-3-12B-IT (multimodal architecture)
# ---------------------------------------------------------------------------
print("Loading model...")
...
# Gemma 3 is multimodal: config may be nested under text_config
_cfg = getattr(model.config, "text_config", model.config)
NUM_LAYERS = _cfg.num_hidden_layers
_HIDDEN = _cfg.hidden_size
assert _HIDDEN == HIDDEN_DIM, f"Expected hidden_dim={HIDDEN_DIM}, got {_HIDDEN}"
print(f"Loaded {MODEL_ID} on {DEVICE}, {NUM_LAYERS} layers, hidden_dim={HIDDEN_DIM}")
...
def generate(prompt, seed=SEED, max_new_tokens=GEN_MAX_TOKENS):
    ...
    # Gemma 3 returns a dict (BatchEncoding), Gemma 2 returns a tensor
...
def extract_per_token_activations(text, layers, max_len=512):
    ...
    for layer in layers:
        ...
        # Verify no NaN/inf (int4 had this issue; bf16 should be clean)
...
# ---------------------------------------------------------------------------
# Probe Architectures (identical to exp8)
# ---------------------------------------------------------------------------
...
class AttentionClassProbe(nn.Module):
    ...
    Paper spec for 12B: E=2048, Nh=16.
    """
    ...
    def forward(self, x_layers, mask=None):
        ...
        return self.wout(value).squeeze(-1)


# ---------------------------------------------------------------------------
# Datasets (identical to exp8)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 9.1: Generate completions
# ---------------------------------------------------------------------------
def stage_generate():
    ...
    print("\n" + "=" * 60)
    print("  Stage 9.1: Generate Completions")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.2: Extract + verify entities
# ---------------------------------------------------------------------------
def stage_extract_verify(completions):
    ...
    print("\n" + "=" * 60)
    print("  Stage 9.2: Extract & Verify Entities")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.3: Extract per-token activations
# ---------------------------------------------------------------------------
def stage_extract_activations(completions):
    ...
    print("\n" + "=" * 60)
    print("  Stage 9.3: Extract Per-Token Activations")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.4: Prepare training/test data
# ---------------------------------------------------------------------------
def stage_prepare_data(completions, entities, activation_data):
    print("\n" + "=" * 60)
    print("  Stage 9.4: Prepare Training Data")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.5a: Train localization probe
# ---------------------------------------------------------------------------
def train_localization(loc_train, loc_test):
    print("\n" + "=" * 60)
    print("  Stage 9.5a: Train Localization Probe")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.5b: Train classification probe
# ---------------------------------------------------------------------------
def train_classification(cls_train, cls_test):
    print("\n" + "=" * 60)
    print("  Stage 9.5b: Train Classification Probe")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.6: End-to-end evaluation
# ---------------------------------------------------------------------------
def stage_evaluate(loc_probe, cls_probe, data, threshold=0.7):
    print("\n" + "=" * 60)
    print("  Stage 9.6: End-to-End Evaluation")
    print("=" * 60)
...
# ---------------------------------------------------------------------------
# Stage 9.7: Ablation -- linear probe on same data
# ---------------------------------------------------------------------------
def stage_ablation(data):
    print("\n" + "=" * 60)
    print("  Stage 9.7: Ablation - Linear Probe on Same Data")
    print("=" * 60)
...
def generate_report(results, ablation, data_stats, elapsed):
    ...
    lines = [
        "# Experiment 9: Proper Goodfire Replication at Paper Scale (12B)",
        "",
        ...
        "## Methodology",
        "",
        "Same as Exp 8, but on the paper's actual model (Gemma-3-12B-IT).",
        ...
        f"{len(PROMPTS) * N_COMPLETIONS}",
        f"- **Verifier:** Claude CLI",
        f"- **Layers:** Loc=L{LOC_LAYER}, Cls=L{CLS_LAYERS} (paper's actual layers)",
        "",
        "## Data Statistics",
        "",
    ...
    if cls:
        ...
        lines += [
            ...
            "",
            "| Metric | Paper (12B) | Exp 8 (2B) | Exp 9 (12B) |",
            "|--------|-------------|------------|-------------|",
            f"| AUROC | 0.94 | 0.762 | **{cls['auroc']:.4f}** [{ci[0]:.3f}, {ci[1]:.3f}] |",
            f"| Precision (tau=0.7) | 0.85 | 0.746 | **{t7.get('precision', 0):.3f}** |",
            f"| Recall (tau=0.7) | 0.56 | 0.686 | **{t7.get('recall', 0):.3f}** |",
            f"| F1 (tau=0.7) | -- | 0.715 | **{t7.get('f1', 0):.3f}** |",
            f"| Optimal tau | -- | 0.50 | {opt.get('threshold', 0):.2f} "
    ...
    if loc:
        lines += [
            ...
            "",
            f"| Metric | Paper (12B) | Exp 8 (2B) | Exp 9 (12B) |",
            "|--------|-------------|------------|-------------|",
            f"| AUROC | 0.88 | 0.976 | **{loc['auroc']:.4f}** |",
            "",
        ]
    ...
    if lr:
        ...
        lines += [
            "## Ablation: Architecture vs Data vs Scale",
            "",
            "| Probe | AUROC | Notes |",
            "|-------|-------|-------|",
            f"| Attention 12B (this exp) | **{cls_auroc:.4f}** | Paper model + paper arch |",
            f"| Linear 12B (ablation) | {lr['auroc']:.4f} | Paper model + simple arch |",
            "| Attention 2B (exp8) | 0.762 | Small model + paper arch |",
            "| Linear 2B (exp8 ablation) | 0.779 | Small model + simple arch |",
            "| Linear TruthfulQA (exp4 OOD) | 0.592 | Wrong data + small model |",
            "",
            f"Scale effect (12B vs 2B, attention): {cls_auroc - 0.762:+.3f} AUROC",
            f"Scale effect (12B vs 2B, linear): {lr['auroc'] - 0.779:+.3f} AUROC",
            f"Architecture effect (12B): {cls_auroc - lr['auroc']:+.3f} AUROC",
            "",
        ]
    ...
    cls_auroc = cls.get("auroc", 0.5)
    if cls_auroc > 0.9:
        lines.append("The 12B model substantially improves classification. Model scale "
                      "was the missing ingredient. The Goodfire methodology replicates.")
    elif cls_auroc > 0.8:
        lines.append("Model scale helps significantly but doesn't fully close the gap. "
                      "Data volume (50 prompts vs 20K) likely accounts for the remainder.")
    elif cls_auroc > 0.7:
        lines.append("Modest improvement from scale. The gap to 0.94 remains large. "
                      "Data volume is likely the dominant remaining factor.")
    else:
        lines.append("Model scale alone does not fix the problem. The 12B model does not "
                      "encode substantially better truthfulness representations than 2B.")
    lines.append("")
    return "\n".join(lines)
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("  Experiment 9: Proper Goodfire Replication (12B)")
    print("=" * 60)
    ...
    json_results = {
        ...
        "paper_comparison": {
            ...
            "exp8_cls_auroc": 0.762, "exp8_loc_auroc": 0.976,
            "exp8_linear_auroc": 0.779,
        },
    ...
    probe_path = REPO_ROOT / "results" / "exp9_probes.pkl"
